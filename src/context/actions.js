...
const authorise = (context, params, setLoading) => {
    ...
    if(!state.isAuthed) { 
        ...
        localStorage.setItem('access_token', JSON.stringify({ access_token: access_token, expiry: authTime.getTime() + ((expires_in-200)*1000) }));
...
const fetchTracks = (context, setLoading) => {
    ...
    if(!state.top_tracks) {
        ...
        spotifyApi.getMyTopTracks({ time_range: 'long_term', limit: 50 })
            ...
            dispatch({ type: 'SPOTIFY_TRACK', data: Object.create({
                'short': res[0].items,
                'medium': res[1].items,
                'long': res[2].items
            })})
...
const fetchArtists = (context, setLoading) => {
    ...
    if(!state.top_artists) {
        Promise.all([
            ...
            dispatch({ type: 'SPOTIFY_ARTIST', data: Object.create({
                'short': data[0].items,
                'medium': data[1].items,
                'long': data[2].items
            })})
...
export { fetchTracks, fetchRecent, fetchUser, fetchArtists, authorise, reauthorise }